...
Mat detectCrop(Mat &img, CascadeClassifier &cascade,
...
void validate_on_lfw_data(LightFaceRecognizer &recognizer) {
  ...
  while (getline(pairs_file, line)) {
    ...
    Mat face1_cropped = detectCrop(face1, cascade, recognizer);
    Mat face2_cropped = detectCrop(face2, cascade, recognizer);
    ...
    // float distance = recognizer.CalculateDistance(face1_feature,
    // face2_feature);
    float distance =
...
void validate_on_lfw_funneled_data(LightFaceRecognizer &recognizer) {
  ifstream pairs_file("../../lfw_data/pairs.txt");
  // ifstream pair_file("../../lfw_data/COLOR224issame.txt");
  ofstream distance_file("distance_file.txt");
  // initialize parameters
  ...
  while (getline(pairs_file, line)) {
    ...
    // Load face images
    std::ostringstream face1_string_stream;
    face1_string_stream << "../../lfw_data/lfw_funneled/" << name1 << "/"
                        << name1 << "_" << setw(4) << setfill('0') << id1
                        << ".jpg";
    std::string face1_name = face1_string_stream.str();
    std::ostringstream face2_string_stream;
    face2_string_stream << "../../lfw_data/lfw_funneled/" << name2 << "/"
                        << name2 << "_" << setw(4) << setfill('0') << id2
                        << ".jpg";
    std::string face2_name = face2_string_stream.str();
...
// void validate_on_lfw_funneled_data_medium(MediumFaceRecognizer &recognizer) {
// ifstream pairs_file("../../lfw_data/pairs.txt");
//// ifstream pair_file("../../lfw_data/COLOR224issame.txt");
// ofstream distance_file("distance_file.txt");
//// initialize parameters
...
//// int csize = 120;
//// int half_padding = (face1.cols - csize) / 2;
...
//// Mat resized_face;
//// resize(face1_cropped, resized_face, Size(100,100));
//// imshow("cropped face", resized_face);
//// waitKey(0);
//// Mat face1_cropped(face1);
//// Mat face2_cropped(face2);
...
void validate_on_prepared_data(LightFaceRecognizer &recognizer) {
  ...
  while (pair_file >> gt) {
    ...
    // float distance = recognizer.CalculateDistance(face1_feature,
    // face2_feature);
    // float distance = recognizer.CalculateEculideanDistance(face1_feature,
    // face2_feature);
    float distance =
        recognizer.CalculateCosDistance(face1_feature, face2_feature);
...
int main(int argc, char **argv) {
  ...
  // true);
  // MediumFaceRecognizer recognizer(
  //"../../face_rec_models/model_cnn/medium",
  //"pool5",
  // false);

  validate_on_lfw_data(recognizer);
  //validate_on_lfw_funneled_data(recognizer);
  // validate_on_prepared_data(recognizer);

  // validate_on_lfw_funneled_data_medium(recognizer);
